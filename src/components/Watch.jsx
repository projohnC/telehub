...
export default function WatchTrailer(props) {
  ...
  // Auto-hide subtitle button when there's no mouse/keyboard gesture
  // (mirrors Plyr's own control auto-hide behavior).
  ...
  // Unique key for the currently playing content. Subtitles are scoped to this
  // key so switching to a different movie/series/episode never resurrects the
  // previous title's subtitle track.
  const contentKey = (() => {
    const base =
      props.id?.imdb_id ||
      props.id?._id ||
      ...
      props.id?.name ||
      "";
    if (props.popUpType === "episode") {
      return `ep:${base}:s${props.seasonNumber ?? ""}:e${props.episodeNumber ?? ""}`;
    }
    return `mv:${base}`;
  })();
  ...
  const clearSubtitles = useCallback(() => {
    ...
    savePreference(null, contentKey);
  ...
  const applySubtitle = useCallback(
    async ({ src, label, lang, persist = true, silent = false }) => {
      try {
        ...
        setIsSubtitlesModalOpen(false);
        if (persist) savePreference({ src, label, lang: lang || "en" }, contentKey);
    ...
    [ensureOverlay, getVideoEl, renderCue, contentKey]
  );

  // Whenever the content changes (different movie/series/episode), drop any
  // cues from the previous title and only restore a preference that was saved
  // for this exact content.
  useEffect(() => {
    // Clear stale overlay/cues from the previous title immediately.
    cuesRef.current = [];
    setActiveSubtitleName("");
    if (overlayRef.current) {
      overlayRef.current.innerHTML = "";
    ...
    const pref = readPreference(contentKey);
  ...
  // Show subtitle button on mouse/keyboard activity within the player,
  // hide after a short idle timeout — matches Plyr's control auto-hide.
  useEffect(() => {
    ...
    const show = () => {
      ...
      clearTimeout(timer);
      timer = setTimeout(() => setIsControlsVisible(false), 2500);
    };
    const hideNow = () => {
      clearTimeout(timer);
    ...
    host.addEventListener("mousedown", show);
    host.addEventListener("touchstart", show, { passive: true });
    host.addEventListener("mouseleave", hideNow);
    window.addEventListener("keydown", show);
    return () => {
      clearTimeout(timer);
      host.removeEventListener("mousemove", show);
      host.removeEventListener("mousedown", show);
      host.removeEventListener("touchstart", show);
      host.removeEventListener("mouseleave", hideNow);
      window.removeEventListener("keydown", show);
    };
  }, [sources.length, isModalOpen, props.isInline]);
  ...
  const subtitlesButton = (
    <button
      ...
      } ${isControlsVisible || isSubtitlesModalOpen ? "opacity-100" : "opacity-0 pointer-events-none"}`}
    >
